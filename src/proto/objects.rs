...
use crate::proto::ClientEvent::ServerData;
...
pub fn parse_baseline<T: AsRef<[u8]>>(cur: &mut Cursor<T>) -> Option<ClientEvent> {
    ...
    return parse_delta_entity(number, bits, cur);
}

fn parse_delta_entity<T: AsRef<[u8]>>(entnum: i16, bits: u32, cur: &mut Cursor<T>) -> Option<ClientEvent> {
    Some(ClientEvent::DeltaEntity(DeltaEntity {
        number: entnum,
        model_index: if bits & EntityStateBits::MODEL  != 0 { Some(cur.read_u8().ok()?) } else { None },
        model_index2: if bits & EntityStateBits::MODEL2  != 0 { Some(cur.read_u8().ok()?) } else { None },
        model_index3: if bits & EntityStateBits::MODEL3  != 0 { Some(cur.read_u8().ok()?) } else { None },
        model_index4: if bits & EntityStateBits::MODEL4  != 0 { Some(cur.read_u8().ok()?) } else { None },
        frame: if bits & EntityStateBits::FRAME8  != 0 && bits & EntityStateBits::FRAME16 != 0 {
            ...
            // only F16 is set
            Some(cur.read_i16::<LittleEndian>().ok()?)
        } else { None }, // neither is set
        skin: if bits & (EntityStateBits::SKIN8 | EntityStateBits::SKIN16) == (EntityStateBits::SKIN8 | EntityStateBits::SKIN16) {
        ...
        effects: if bits & (EntityStateBits::EFFECTS8 | EntityStateBits::EFFECTS16) == (EntityStateBits::EFFECTS8 | EntityStateBits::EFFECTS16) {
            Some(cur.read_u32::<LittleEndian>().ok()?) // laser
        ...
        render_fx: if bits & (EntityStateBits::RENDERFX8 | EntityStateBits::RENDERFX16) == (EntityStateBits::RENDERFX8 | EntityStateBits::RENDERFX16) {
        Some(cur.read_u32::<LittleEndian>().ok()?) // laser
        ...
        origin0: if bits & EntityStateBits::ORIGIN1 != 0 { parse_coord(cur) } else { None },
        origin1: if bits & EntityStateBits::ORIGIN2 != 0 { parse_coord(cur) } else { None },
        origin2: if bits & EntityStateBits::ORIGIN3 != 0 { parse_coord(cur) } else { None },
        angle0:  if bits & EntityStateBits::ANGLE1  != 0 { parse_angle(cur) } else { None },
        angle1:  if bits & EntityStateBits::ANGLE2  != 0 { parse_angle(cur) } else { None },
        angle2:  if bits & EntityStateBits::ANGLE3  != 0 { parse_angle(cur) } else { None },
        old_origin0: if bits & EntityStateBits::OLDORIGIN != 0 { parse_coord(cur) } else { None },
        old_origin1: if bits & EntityStateBits::OLDORIGIN != 0 { parse_coord(cur) } else { None },
        old_origin2: if bits & EntityStateBits::OLDORIGIN != 0 { parse_coord(cur) } else { None } ,
        sound: if bits & EntityStateBits::SOUND != 0 { cur.read_u8().ok() } else { None },
        event: if bits & EntityStateBits::EVENT != 0 { cur.read_u8().ok()? } else { 0 },
